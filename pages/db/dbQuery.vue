    ...
    <el-row :gutter="24" type="flex" class="condition-wrap">
      <el-col :span="6" class="condition-item">
        <div class="action-wrap">
          ...
          <div class="actions">
            <div @click="requestTypeList"><i class="el-icon-refresh"></i></div>
      ...
      <el-col :span="12" v-show="!isTypeSetting" class="condition-item">
        ...
        <div class="sql-form-item">
          <div class="label">{{$t('报表.标题')}}: {{form.title}}</div>
...
export default {
  ...
  computed: {
    ...
    canRunSql(){
      ...
      return !!form.sqlStr&&this.selectedType.id&&this.selectedSql.id;
    }
  },
  watch: {
    filterType(val){
  ...
  methods: {
    async init(){
      await this.requestTypeList();
      // await this.requestRoleList();
    },
    async requestTypeList(){
      try{
        let {data} = await axios.get('/api/json/voljin/admin/db/typelist');
        if(data.success){
    ...
    download(){
      this.$confirm(this.$t('报表.确定下载'), this.$t('通用.提示'), {
        ...
        let {data} = await axios.post(`/api/json/voljin/admin/db/download/${this.selectedType.id}/${this.selectedSql.id}`, params, {formData: true});
        if(data.success){
